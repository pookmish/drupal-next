...
export const Banner = ({su_banner_body= null, su_banner_button= null, su_banner_header= null, su_banner_image= null, su_banner_sup_header= null, behavior_settings= null}) => {
  const imageUrl = su_banner_image?.field_media_image?.uri?.url;
  const hasCardText = su_banner_button || su_banner_header || su_banner_sup_header || su_banner_body;
  return (
    <div className="hero su-basefont-23 su-relative su-h-full su-max-h-500 su-mx-auto">
      ...
      {hasCardText &&
          <div
              className={"card su-block su-basefont-23 su-leading-display su-bg-white su-text-black su-border su-border-solid su-border-black-10 su-shadow su-relative su-max-w-full md:su-absolute md:su-top-auto md:su-bottom-36 " + (behavior_settings?.hero_pattern?.overlay_position === 'right' ? 'md:su-right-36' : 'md:su-left-36')}>
              <Card
